 ...
 * @author Denis
 */
public class LClassesExhaustionBigLexDualSimplexTableau<ValueField extends FieldElement<ValueField>>
        extends ZeroConstraintsLexDualSimplexTableau<ValueField>
        implements LClassesExhaustionSimplexTableau<ValueField> {
...
//            matrix.setEntry(getObjectiveFunctionsOffset(), getNumRightHandSide(), -1.0);
    ...
    public int appendRecordRow(Double recordValue, Double delta) {
        ...
        ValueField[] recordRow = getFieldHelper().buildArray(getWidth());
    ...
    protected void fillRecordRow(Double recordValue, Double delta) {
        for (int j = 0; j < getWidth(); j++) {
            if (j < getNumRightHandSide()) {
                setEntry(recordRowIndex, j, getFieldHelper().fromDouble(-(recordValue + delta)));
            } else {
                setEntry(recordRowIndex, j,
                        getFieldHelper().fromDouble(-getObjectiveFunction().getCoefficients().getEntry(j - getNumRightHandSide())));
            }
        }
    }

    public int appendConstraintToVariableColumn(int variableIndex, ValueField constraint) {
        ValueField[] row = getFieldHelper().buildArray(getWidth());
//        Arrays.fill(row, 0.0);
        row[variableIndex + getNumRightHandSide()] = getFieldHelper().getOne();
